...
describe('ProjectService', () => {
  ...
  it('ensureOwner — пропускає власника', async () => {
    projects.findMember.mockResolvedValue(dummyMember(ProjectRole.OWNER));
  ...
  it('addMember — конфлікт, якщо вже член', async () => {
    projects.findMember
      .mockResolvedValueOnce(dummyMember(ProjectRole.OWNER)) // ensureOwner
      .mockResolvedValueOnce(dummyMember(ProjectRole.MEMBER, 'u2')); // findMember для нового
  ...
  it('removeMember — не дозволяє видалити власника', async () => {
    projects.findMember
      .mockResolvedValueOnce(dummyMember(ProjectRole.OWNER))
      .mockResolvedValueOnce(dummyMember(ProjectRole.OWNER, 'u1'));
